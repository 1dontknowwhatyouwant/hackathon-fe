      ...
      <div className="flex min-h-full flex-col">
        <LuxuryReveal>
          <ScreenHeader
            ...
            title="제품 정보 확인"
            description="인식된 정보를 확인하고 수정하세요"
          />
        </LuxuryReveal>
        ...
        <form className="mt-6 flex flex-1 flex-col" onSubmit={handleSubmit}>
          <LuxuryReveal delay={60}>
            <div
              role="img"
              aria-label={photoPreviewUrl ? "등록할 제품 사진" : "제품 사진 없음"}
              className="size-[110px] rounded-[16px] bg-[#e8e3d9] bg-cover bg-center"
              style={
                photoPreviewUrl
          ...
          <LuxuryReveal className="mt-8 space-y-3" delay={110}>
            <input
              aria-label="제품명"
              value={draft.name}
              maxLength={50}
              placeholder="제품명 · Aren Shopper"
            ...
            <div className="relative">
              <input
                ...
                className={`${fieldClassName} cursor-pointer ${
                  draft.purchaseDate ? "" : "text-transparent"
                }`}
                onChange={(event) =>
              ...
              />
              {!draft.purchaseDate ? (
                <span className="pointer-events-none absolute inset-y-0 left-[17px] flex items-center text-[13px] text-[#6e707a]">
                  구매일 · 선택
                </span>
              ) : null}
            </div>
